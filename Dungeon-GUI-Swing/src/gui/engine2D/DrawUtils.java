...
public class DrawUtils {
	...
	public static void drawFlipped(Graphics g, Image im, Rectangle o) {
		BufferedImage mBufferedImage = new BufferedImage(im.getWidth(null),
				im.getHeight(null), BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2 = mBufferedImage.createGraphics();
		g2.drawImage(im, null, null);

		BufferedImage mBufferedImage2 = new BufferedImage(im.getWidth(null),
				im.getHeight(null), BufferedImage.TYPE_INT_ARGB);
	...
	public static void fillGraphicObject(GraphicObject o, Graphics g) {
		...
		if (image == null) {
			...
			g.fillRect(rect.getX(), rect.getY(), rect.getWidth(),
					rect.getHeight());
			if (o.getRim()
					|| ((clickedObject instanceof Door) && (((Door) clickedObject)
							.hasLock()))) {
				g.setColor(Color.black);
				g.drawRect(rect.getX(), rect.getY(), rect.getWidth(),
						rect.getHeight());
			}
		} else {
			g.setColor(Color.RED);
			g.drawImage((Image) image.getImage(), rect.getX(), rect.getY(),
