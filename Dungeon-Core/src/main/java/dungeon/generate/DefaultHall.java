...
public class DefaultHall extends AbstractHall {
	...
	public RoomQuest_XxY insertRQXY(int x, int y, boolean locked, Shrine s) {
		...
		while (!ok) {
			...
			if ((!this.isHereRimRoom(r, 0))
				...
				rq = new RoomQuest_XxY(x, y, r.getNumber(), sec.df, locked);
				ok = rq.isPossible();
				if(x == 1 && y == 1) {
					if(!ok) {
	...
	private boolean isRimRoom(Room r, int dir) {
		...
		List<Room> neighbours = dungeon.getNeighbourRooms(r);
...
JDDimension dim,
	...
	private boolean testRow(JDPoint start, int dir, int dist) {
		...
		int k = 0;
		Room toTest = dungeon.getRoom(start);
		if (toTest == null) {
		...
		while (valid && (k < dist)) {
			k++;
			toTest = dungeon.getRoom(JDPoint.walkDir(toTest.getNumber(), dir, 1));
	...
	private int walkTo(JDPoint start, int dir) {
		Room startRoom = dungeon.getRoom(start);
		int k = 0;
		Room toTest = dungeon.getRoomAt(startRoom, RouteInstruction.direction(dir));
		boolean valid = testRoom(toTest);
		while (valid) {
			k++;
			toTest = dungeon.getRoomAt(toTest, RouteInstruction.direction(dir));
			valid = testRoom(toTest);
