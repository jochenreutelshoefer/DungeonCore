...
public abstract class Figure extends DungeonWorldObject
	...
	private String getStatusText(HealthLevel level) {
		if (level == HealthLevel.Strong || level == HealthLevel.Good) {
			return " " + JDEnv.getResourceBundle().getString("status_strong");
		}
		else if (level == HealthLevel.Ok) {
			return " " + JDEnv.getResourceBundle().getString("status_struck");
		}
		else if (level == HealthLevel.Injured) {
			return " "
					+ JDEnv.getResourceBundle().getString("status_wounded");
		}
		else if (level == HealthLevel.Weak) {
			return " " + JDEnv.getResourceBundle().getString("status_weak");
		}
		else if (level == HealthLevel.Dying) {
			return " " + JDEnv.getResourceBundle().getString("status_done");
		}
		if (dead) {
			return " " + JDEnv.getResourceBundle().getString("status_dead");
		}
		return null;
	}

	public String getStatus() {
		return this.getStatusText(this.getHealthLevel());
	}
