...
import de.jdungeon.ai.DefaultHeroReflexBehavior;
...
public class Hero extends Figure implements InfoProvider, Serializable {
    ...
    private HeroCategory heroCategory;
    ...
    /**
     * Copy constructur for hero backup (restart after death)
     *
     * @param hero hero to be copied
     */
    public static Hero copy(Hero hero) {
        Hero copy = new Hero(HeroCategory.fromValue(hero.getHeroCode()));
        Character character = new Character(hero.getCharacter(), copy);
        copy.setC(character);
        Inventory inventory = Inventory.copy(hero.getInventory(), copy);
        copy.setInv(inventory);
        copy.setSpellbook(hero.getSpellbook().copy());
        return copy;
    }

    public Hero(HeroCategory category, Character character, Inventory inventory) {
    ...
    public Hero(HeroCategory category) {
        this.heroCategory = category;
        this.reflexReactionUnit = new DefaultHeroReflexBehavior(this);
    }
