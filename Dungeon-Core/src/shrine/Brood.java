...
public class Brood extends Shrine {
	...
	private Monster spitMonster(int worth) {
		...
		if (type==Brood.BROOD_NATURE) {
			if (i < 1) {
				return new Wolf(worth,this.location.getDungeon());
			} else {
				return (
					new Spider(worth,this.location.getDungeon()));
			}
		} else if (type == Brood.BROOD_CREATURE) {
			if (i < 1) {
				return 
					new Orc(
						worth,this.location.getDungeon()
						);
			} else {
				return (new Ogre(worth,this.location.getDungeon()));
			}
		}
		if (type == Brood.BROOD_UNDEAD) {
			if (i < 1) {
				...
				return new Ghul(worth,this.location.getDungeon());
			}
		} else
			return null;
