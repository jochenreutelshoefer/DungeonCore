 ...
 */
public class SimpleHeroBehavior extends AI implements ControlUnit {
	
	HeroInfo h;
	public SimpleHeroBehavior(HeroInfo h) {
		this.h = h;
		this.att = new AttitudeDefaultHero();
		//h = i;
	}
	...
	public Action chooseMovementAction() {
		...
		if(h.getActionPoints() < 2) {
			return Action.makeEndRoundAction(/*h.getFighterID()*/);
		}
	...
	public Action chooseFightAction(){
		...
		if(this.h.getHealthLevel() <= 2 && Math.random() < 0.3) {
			...
			a =  Action.makeActionAttack(/*h.getFighterID(),*/0);
			this.h.checkMovementAction(a);
