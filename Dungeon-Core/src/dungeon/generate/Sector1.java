...
public class Sector1 extends Sector {
	...
	public Sector1(Dungeon d, JDPoint startRoom, int number,
		...
		mainHall = new Hall(d, mainSize, 800, 0, startRoom,
				RouteInstruction.NORTH, 3, this, "main", 0);
		...
		Hall normalRaum = makeHall(0, 5, 100, 10, 0, 0, null,
		...
		Hall wolfRaum = makeHall(Monster.WOLF, 5, 1200, 6, 0, 0, df
				.getNextKey(), 0, "Wolf", 1);
		halls.add(wolfRaum);
		...
		Hall orcRaum = makeHall(Monster.ORC, 5, 1200, 6, 0, 0, df.getNextKey(),
				0, "Ork", 2);
		halls.add(orcRaum);
		...
		Hall skeletonRaum = makeHall(Monster.SKELETON, 5, 1200, 6, 0, 0, df
	...
	private Hall makeVimHome(Room r1, Key key) {
		Room toMake = null;
	...
	private Hall makeTreasureRoom(Room r1, Key key) throws DungeonGenerationFailedException{
	...
	private Hall doHall(Room r1, int monsterType, int monsterCnt,
		...
		Hall halle1 = new Hall(d, hallSize, monsterValue, subHalls, toMake
				.getNumber(), dir, roomQuests, this, name, floorIndex);
		// System.out.println("Versuche halle zu machen: ");
		boolean ok = halle1.makeArea(null);
		if (ok == false) {
			return null;
	...
	private Hall makeHall(int monsterType, int monsterCnt, int monsterValue,
		...
		Hall h1 = doHall(r1, monsterType, monsterCnt, monsterValue, hallSize,
	...
	public boolean setRoomQuest(String roomQuestType, Hall theHall, Shrine s) {
		...
		if (roomQuestType.equals("2x2_1")) {
			rq = new RoomQuest_2x2_1(rq_point, df, restItems, rareItems);
		}
