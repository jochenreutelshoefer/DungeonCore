...
public abstract class DungeonFiller {
	...
	private final List<Key> keyList = new LinkedList<Key>();

	private static String[] keyStrings = { "Kupfer", "Eisen", "Silber", "Gold",
			"Platin",
			"Bronze", "Blech", "Stahl", "Piponium" };

	public DungeonFiller(Dungeon d, Rune[] runen) {
		this.d = d;
		this.runen = runen;
		this.map = getMap();
	...
	// Der Sektor wo die zuf�lligen Monster in einen zuf�lligen Raum reinkommen
			int size, // Code fuer die groe�e (gro�/klein) 1=kleine 2= grosse
						// 0=zuf�llig
	...
	private void setDoors(Room r) {
		// Nordlicher Raum
		Room north = d
				.getRoomNr(r.getNumber().getX(), r.getNumber().getY() - 1);
		if (north != null) {
